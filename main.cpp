        ...
        if (event == "telemetry") {
          ...
          for (auto &car_reading: sensor_fusion){
            ...
            if (d < (LANE_WIDTH  + LANE_WIDTH * lane) && d > (LANE_WIDTH * lane) ){
              ...
              double vx = car_reading[3];
              double vy = car_reading[4];

              double speed = sqrt(vx * vx + vy * vy);

              double check_car_s = car_reading[5];
          ...
          vector <double> next_wp1 = getXY(car_s+40, LANE_WIDTH/2 + (LANE_WIDTH * lane) , map_waypoints_s , map_waypoints_x , map_waypoints_y);
          vector <double> next_wp2 = getXY(car_s+50, LANE_WIDTH/2 + (LANE_WIDTH * lane) , map_waypoints_s , map_waypoints_x , map_waypoints_y);
